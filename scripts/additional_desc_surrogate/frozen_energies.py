...
def main_function():
    ...
    for file in xyz_frozen_files:
        run_xtb(file)
    os.chdir('../scripts/additional_desc_surrogate')

    all_bdfe_frozen = multiprocess_func(get_energy_parallel, paton_rxns.iloc, 70)

    paton_rxns['BDE_fr'] = all_bdfe_frozen[0]
    paton_rxns = paton_rxns.loc[paton_rxns['BDE_fr'] < 1000]
    paton_rxns.to_csv('../../data/paton_rxns_frozen_BDE.csv')
