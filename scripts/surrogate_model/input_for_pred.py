...
parser.add_argument('--csv-file', type=str, default='../../data/reactivity_database_corrected.csv',
                    help='path to file containing the successful profiles')


def create_input_pred(csv_file):
    ...
    df_mapped['rxn_id'] = df['rxn_id']
    df_mapped['rxn_smiles'] = rxns_mapped
    df_mapped['G_r'] = df['dG_rxn']
    df_mapped['DG_TS'] = df['dG_act']
    df_mapped['DG_TS_tunn'] = df['dG_act_corrected']

    df_mapped = df_mapped[df_mapped['rxn_smiles'] != False]
    df_mapped.reset_index(inplace=True)
    df_mapped.drop(columns=['index'], inplace=True)
    df_mapped.to_csv('../../data/reactivity_database_mapped.csv')
    ...
    df_smiles.to_csv('../../data/species_reactivity_acs_omega_dataset.csv')
