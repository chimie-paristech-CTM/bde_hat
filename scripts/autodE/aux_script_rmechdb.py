...
def rxn_completed(idx : str) -> bool:
    ...
    thermal_dir = os.path.join(pwd, 'reaction/thermal')
    sp_dir = os.path.join(pwd, 'reaction/single_points')

    if os.path.isdir(out_dir):
        if check_TS(out_dir):
            if check_thermal(thermal_dir) and check_sp(sp_dir):
                return True
            else:
                mssg = f"rxn {idx} finished, error total of files"
        else:
...
def check_sp(path):
    """ Check if all thermal calculations finished"""

    files = glob(f"{path}/*")

    return True if (len(files) == 10) else False


def check_thermal(path):
    """ Check if all thermal calculations finished"""

    files = glob(f"{path}/*")

    return True if (len(files) in [8, 6]) else False
...
def copy_ts_file(idx : str, dst_dir : str, ts_name : str) -> None:
    ...
    freq_xyz = float(comment_line.split()[12])
...
def check_copies() -> bool:
    ...
    return True if files not in [17, 16] else False   # 17 because 6 .xyz + 1 .csv + 5 freq .log + 5 sp .log
