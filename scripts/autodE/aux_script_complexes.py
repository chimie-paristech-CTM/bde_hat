...
def check_sp(path):
    ...
    return True if (len(files) == 14) else False
...
def check_thermal(path):
    ...
    return True if (len(files) == 12) else False
...
def check_copies() -> bool:
    ...
    return True if files != 23 else False  # 23 because 8 .xyz + 1 .csv + 7 freq .log + 7 sp .log
